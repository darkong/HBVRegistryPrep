...
* Created : 		January 14, 2013 by Alexia Exarchos
* Last Updated : 	X by X
...
*HBV cases reported in 2011 by LHJ;
proc freq data=main01;
table first_LHJ*firstyear/list missing norow nocum;
where firstyear = 2011 and first_lhj ne "OUT OF STATE";
run;

*HBV cases by age; 
proc freq data=main01;
table first_lhj*age/missing norow nocum;
format age agechart.;
where firstyear=2011 and first_lhj ne "OUT OF STATE";
run;
...
if race_ethnicity in ('Other','Unknown','Multirace') then race_ethnicity = 'Other/Multi/Unk               ';

run;

*HBV cases by race/ethnicity;
proc freq data=race;
table first_lhj*race_ethnicity/missing norow nocum;
...
*HBV cases by gender;
proc freq data=main01;
table first_lhj*sex/missing norow nocum;
where firstyear=2011 and first_lhj ne "OUT OF STATE";
run;
